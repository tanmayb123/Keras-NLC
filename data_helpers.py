#NOTE: ORIGINAL OWNER OF FILE (ON GITHUB) DELETED THEIR REPO. UNABLE TO FIND OWNER.
import numpy as np
import re
import itertools
...
def filterinput(raw_comment):
    raw_comment_cut = raw_comment.split()
    sentence_padded = pad_sentence(raw_comment_cut)
    vocabulary, vocabulary_inv = build_vocab(sentence_padded)
    x = build_input_data_for_sentences(sentence_padded, vocabulary)
    return x
